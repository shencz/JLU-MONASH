...
x = np.loadtxt('x_test_nft3.txt')
y = np.loadtxt('y_test_nft3.txt')
...
for i in range(len(gcs)):
    if i > sentence_len-1 and i < 798 - pad_len:
        # print(text[i])
        # print(gcs[i])
        for j in range(len(x[t])):
            if text[i][0] == x[t][j] and text[i][1] == x[t][j+1] and text[i][2] == x[t][j+2]:
                # print(x[t][j], x[t][j+1])
                # print(gcs[j], gcs[j+1])
                # print(j)
                gcs[j] = gcs[j] + gcs[i]
                gcs[j+1] = gcs[j+1] + gcs[i]
                gcs[j+2] = gcs[j+2] + gcs[i]

prob =[i/4 for i in gcs[:sentence_len]]
prob[0] = prob[0] * 4 / 2
prob[1] = prob[1] * 4 / 3
prob[-2] = prob[-2] * 4 / 3
prob[-1] = prob[-1] * 4 / 2
...
if ans == [[1]]:
    ...
    with open("visualizationft3.html", "w") as html_file:
