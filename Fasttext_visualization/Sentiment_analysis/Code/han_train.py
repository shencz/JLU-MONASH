...
# x = sequence.pad_sequences(x, maxlen=maxlen, padding='post', value=0)
x = zero_pad(x,maxlen)
x = np.array(x)
y = np.array(y)
...
np.savetxt('x_train_rnn.txt', X_train)
np.savetxt('y_train_rnn.txt', y_train)
np.savetxt('x_test_rnn.txt', X_test)
np.savetxt('y_test_rnn.txt', y_test)
...
with tf.name_scope('Metrics'):
    ...
    # Accuracy metric
    sigmoid_val = tf.sigmoid(y_hat)
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.round(sigmoid_val), target_ph), tf.float32))
    tf.summary.scalar('accuracy', accuracy)
    tf.summary.scalar('sigmoid', sigmoid_val)
